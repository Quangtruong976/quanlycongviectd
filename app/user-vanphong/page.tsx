...
export default function UserVanPhong() {
  ...
  function update(index: number, field: keyof Task, value: any) {
    const t = tasks[index];
    // user chỉ chỉnh sửa được tên sản phẩm và ngày hoàn thành của nhiệm vụ admin
              ...
              <tbody>
                {tasks.map((t, i) => (
                  <tr key={i} className={`${t.created_by_user ? "text-green-600" : "opacity-50"}`}>
                    ...
                    <td className="border p-1">
                      <input className="w-full" disabled={!t.isEditing && !t.created_by_user} value={t.ten} onChange={(e) => update(i, "ten", e.target.value)} />
                    </td>
                    <td className="border p-1">
                      <input
                        className={`w-full ${t.created_by_user ? "text-green-600" : "placeholder-red-500"}`}
                    ...
                    <td className="border p-1">
                      <input
                        ...
                        placeholder={!t.ngay_hoan_thanh ? "Nhập ngày hoàn thành" : ""}
                        disabled={!t.isEditing && !t.created_by_user && !t.created_by_user}
